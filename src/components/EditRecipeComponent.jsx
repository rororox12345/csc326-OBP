...
import { updateRecipe, getRecipe } from '../services/RecipesService'
...
const EditRecipeComponent = () => {
    ...
    useEffect(() => {
        ...
        if (id) {
            getRecipe(id).then((response) => {
                ...
                setThisIngredients(response.data.ingredients);
                setThisIngredientAmounts(response.data.ingredientAmounts);
            }).catch(error => {
                console.error(error)
            })
        } else {
    ...
    const getDefaultCheck = (ingredient) => {
        return thisIngredients.indexOf(ingredient.name) !== -1;
    };

    const getDefaultAmount = (ingredient) => {
        const idx = thisIngredients.indexOf(ingredient.name);
        if (idx == -1) {
            return 0;
        } else {
            return thisIngredientAmounts[idx];
        }
    };
                                                    ...
                                                    <td>
                                                        <div className="form-check" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
                                                            <input 
                                                                ...
                                                                defaultChecked={getDefaultCheck(ingredient)}
                                                    ...
                                                    <td>
                                                        ...
                                                        {thisIngredients.indexOf(ingredient.name) != -1 && (
                                                            <input 
                                                                ...
                                                                step="1"
                                                                required
                                                                defaultValue={getDefaultAmount(ingredient)}
